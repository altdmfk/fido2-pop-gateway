...
|---|---|---|---|---|
| **Mode A (표준 JWT 검증)** | 4.36ms | 8.11ms | 0 Bytes | Baseline |
| **Mode B (RSA-2048 PoP)** | 4.87ms | 7.61ms | 499 Bytes | +11.85% |
| **Mode C (ECDSA P-256 PoP)** | 7.27ms | 15.02ms | 243 Bytes | +66.96% |

**기술적 함의 및 하드웨어 시사점**:

1. **페이로드 크기 (RSA vs ECDSA)**: RSA-2048 서명은 256바이트이며 Base64 인코딩 후 약 500바이트의 헤더 오버헤드를 유발합니다. ECDSA P-256 서명은 약 64바이트로, 동일한 보안 강도를 유지하면서 전송 오버헤드를 절반 수준(243 Bytes, 약 51.3% 절감)으로 줄입니다.
...
2. **성능 벤치마크 시각화 (Performance Analytics)**
   ...
   - **페이로드 오버헤드 차트**: RSA-2048 대비 ECDSA P-256 적용 시 얻는 **약 51.3%의 대역폭 절감 효과**를 도넛 차트로 비교 분석합니다.
