...
PoP 게이트웨이 환경은 매 HTTP 요청마다 헤더에 서명을 포함하므로 전송 오버헤드가 누적된다. ECDSA P-256은 RSA-2048 대비 헤더 크기를 50 % 절감하면서도 동등하거나 더 높은 보안 수준을 나타내므로, 본 연구의 서명 알고리즘으로 채택하였다.
...
|---|---|---|---|---|
| **Mode A (표준 JWT)** | 22.26 ms | 26.75 ms | 0 Bytes | 기준 (Baseline) |
| **Mode B (RSA-2048)** | 27.10 ms | 34.77 ms | 499 Bytes | +21.74 % |
| **Mode C (ECDSA P-256)** | 26.19 ms | 34.24 ms | 251 Bytes | +17.61 % |

모드 C는 모드 A 대비 평균 3.93 ms(17.61%)의 지연시간 오버헤드를 나타냈다. 이는 실시간 웹 트래픽 환경에 적용 가능한 수준의 성능으로 판단된다. 95백분위 지연시간에서는 모드 B가 34.77 ms로 가장 높게 측정된 반면, 모드 C는 34.24 ms로 모드 B 대비 안정적인 수치를 기록하였다. 모드 A의 95백분위 지연시간은 26.75 ms로 측정되었다.

페이로드 측면에서는 모드 B가 256 Bytes의 고정 서명 크기와 Base64 인코딩 팽창으로 인해 499 Bytes의 헤더 오버헤드를 나타낸 반면, 모드 C는 64 Bytes의 서명 좌표 구조를 통해 오버헤드가 251 Bytes로 낮게 나타났다. 본 측정은 서명 관련 헤더(`X-FIDO2-Credential-ID`, `X-FIDO2-Signature`, `X-FIDO2-Nonce`, `X-FIDO2-Timestamp`)를 대상으로 하며, 양 알고리즘에 공통으로 추가되는 `X-Body-Digest` 헤더는 비율 비교에서 제외하였다. 이는 RSA 대비 약 50%의 서명 헤더 페이로드 절감 효과에 해당한다.
