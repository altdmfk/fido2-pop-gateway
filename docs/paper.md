...
소지자 토큰(Bearer Token)과 소프트웨어 기반 소유권 증명(DPoP) 규격은 토큰 탈취나 런타임 메모리 침해 환경에서 개인키 유출과 세션 하이재킹을 효과적으로 방어하지 못한다. 본 연구에서는 하드웨어 보안 칩셋에 개인키를 물리적으로 격리하는 FIDO2 규격을 L7 역방향 프록시와 결합한 Proof-of-Possession(PoP) 게이트웨이 아키텍처를 제안한다.  

제안 시스템은 대용량 페이로드의 메모리 부하를 방지하기 위해 스트리밍 다이제스트 파이프라인으로 요청을 정규화하고, 비인가 대량 트래픽 차단을 위해 시간 복잡도 $O(1)$의 슬라이딩 윈도우 인그레스 제어 메커니즘을 적용하였다. 또한 타원곡선 서명 검증을 비동기 스레드 풀로 위임하여 단일 노드의 동시 접속 처리 성능을 향상시켰다. 성능 평가 결과, 제안하는 ECDSA P-256 서명 바인딩은 표준 토큰 검증 대비 평균 2.91 ms(상대 오버헤드 66.96%)의 지연시간 부담을 나타냈으며, RSA-2048 대비 서명 관련 HTTP 헤더 페이로드를 약 51.3% 절감하였다. 아울러 동시성 최적화를 통해 100명의 가상 유저 부하 환경에서도 초당 780건의 처리량(RPS)을 달성하여, 고부하 트래픽 환경에서의 방어 실효성과 실무 적용 가능성을 확인하였다.
...
소프트웨어 계층에서 토큰 재사용을 방지하는 소유권 증명(DPoP) 규격은 매 요청마다 비대칭 서명을 요구한다[2]. 그러나 암호화 키를 브라우저 로컬 저장소나 애플리케이션 메모리에 보관해야 하는 구조적 한계를 지닌다. 호스트 운영체제의 권한 침해나 메모리 덤프가 발생할 경우 저장된 개인키가 추출되어 암호학적 방어가 무력화된다.
...
2. **자원 제어 및 동시성 최적화 아키텍처 설계**
   대용량 요청 본문의 메모리 부하를 방지하는 스트리밍 다이제스트 구조를 설계하였다. 또한 비인가 대량 트래픽 차단을 위해 $O(1)$ 시간 복잡도의 인그레스 제어 메커니즘과 비동기 스레드 풀을 적용하여 고부하 환경의 처리량을 향상시켰다.

3. **정량적 지연시간 및 페이로드 오버헤드의 실증**
   표준 토큰 검증 대비 제안 구조의 지연시간 증가폭을 실측하고, 비대칭 서명 알고리즘 간 헤더 페이로드 오버헤드를 비교하여 제안 시스템의 실무 적용 가능성을 확인하였다.
...
OAuth 2.0 인가 프레임워크의 Bearer 토큰 명세는 토큰 자체의 암호학적 서명 여부만 확인할 뿐, 요청 송신자가 해당 토큰을 발급받은 정당한 주체인지 검증하지 않는다. DPoP는 송신자 제한 토큰 모델을 도입하여 각 HTTP 요청 헤더에 클라이언트가 생성한 비대칭 서명값을 포함하도록 규정한다. 그러나 DPoP 표준은 키의 하드웨어 바인딩을 강제하지 않는다.
...
| HTTP 헤더 인코딩 오버헤드 | 약 499 Bytes | 약 243 Bytes |

PoP 게이트웨이 환경은 매 HTTP 요청마다 헤더에 서명을 포함하므로 전송 오버헤드가 누적된다. ECDSA P-256은 RSA-2048 대비 헤더 크기를 약 51.3% 절감하면서도 동등하거나 더 높은 보안 수준을 나타내므로, 본 연구의 서명 알고리즘으로 채택하였다.
...
본 연구에서는 Dolev-Yao 공격 모델을 확장하여[7] 네트워크 및 호스트 계층에서의 공격자 역량을 정의한다. 공격자는 클라이언트와 게이트웨이 간의 네트워크 트래픽을 도청, 변조, 주입, 지연할 수 있다고 가정한다. 또한 호스트 계층에서 브라우저 스토리지나 운영체제 사용자 공간의 메모리에 접근하여 런타임 토큰 및 세션 아티팩트를 획득한다고 가정한다. 단, 악성코드가 감염 단말에 상주하며 실시간으로 서명 루틴을 대리 호출하는 서명 오라클(Signing Oracle) 공격은 본 연구의 방어 범위에서 제외한다. 본 연구는 탈취한 인증 아티팩트를 원격의 다른 기기에서 오프라인으로 재사용하는 공격을 차단하는 것을 주된 목적으로 한다. 물리적 보안 영역에 대한 훼손 역시 위협 범위에서 제외한다.
...
클라이언트는 인증 절차를 거쳐 JWT와 일회용 난수를 차례로 수신한다. 이후 정규화된 HTTP 요청 다이제스트($SigningPayload$)를 WebAuthn API의 `challenge` 필드에 주입하여 하드웨어 서명을 요청하고, 내부 논리회로에서 생성된 디지털 서명을 수신하여 요청 헤더에 첨부한다. 게이트웨이는 인입된 패킷을 검증한 뒤 유효한 요청에 한하여 업스트림 서비스로 중계한다.
...
$$SigningPayload = Method \parallel Host \parallel Path \parallel Query \parallel \text{SHA-256}(Body) \parallel Nonce \parallel Timestamp$$

각 필드는 줄바꿈 문자(`\n`)로 구분되어 연접되며, 이를 통해 `Path`와 `Query` 간의 경계 모호성을 이용한 충돌(Collision) 공격과 `Host` 변조를 통한 크로스 호스트 공격을 방지한다. 대용량 요청 본문을 처리할 때 게이트웨이의 메모리 부하를 방지하기 위해 스트리밍 다이제스트 파이프라인을 적용한다. 본 메커니즘은 HTTP 청크 인코딩 또는 프록시 계층의 스트림 I/O를 지원하는 환경을 전제로 한다. 게이트웨이는 페이로드 전체를 단일 메모리 버퍼에 적재하지 않고, 청크 단위로 유입되는 네트워크 바이트 스트림을 해시 함수에 즉시 갱신한다. 이를 통해 서명 검증 계층의 해시 상태 유지에 필요한 부가적인 메모리 할당량을 32 Bytes로 고정한다. 단, 게이트웨이는 검증된 요청 본문을 업스트림 서비스로 재전송하기 위해 임시 버퍼에 본문을 일시 보관하므로, 이에 수반되는 추가 메모리는 요청 본문 크기에 비례한다.
...
2. **타임스탬프 및 난수 검증**: 요청 타임스탬프의 허용 오차 범위를 평가하고, 일회용 난수의 존재 여부를 확인 및 즉시 삭제하여 재전송 공격을 방어한다.

3. **JWT 바인딩 검증**: 토큰 내에 `cnf` (Confirmation) 클레임이 존재하는 경우, 현재 제시된 공개키 식별자와 JWT에 바인딩된 식별자의 일치 여부를 추가로 검증하여 공개키 전용 탈취에 의한 위장 접근을 이중으로 차단한다.

4. **비대칭 암호 서명 검증**: 클라이언트가 헤더에 명시한 Claimed Digest를 포함하여 ECDSA P-256 서명의 수학적 유효성을 먼저 검증한다.

5. **본문 다이제스트 스트리밍 대조**: 이전 단계를 통과한 유효한 서명의 요청에 한하여, 본문을 스트리밍하며 실제 해시값과 Claimed Digest의 일치 여부를 대조한다. 이 단계에서 수신 청크는 해시 갱신과 동시에 업스트림 전달용 버퍼에 기록되므로, 스트리밍 검증과 프록시 전달 준비가 단일 순회 내에서 처리된다. 이를 통해 무효한 서명으로 대용량 데이터를 전송하는 자원 고갈 공격을 차단한다.

본 메커니즘의 무결성 및 재전송 차단 효과는 V절의 모의 공격 검증 결과를 통해 확인한다.

### 4. 이중 인그레스 제어 메커니즘

난수 발급 엔드포인트(`/auth/nonce`)를 보호하고 인메모리 상태 저장소에 대한 과도한 부하를 제한하기 위해 2단계 방어 체계를 구성한다.

1. **슬라이딩 윈도우 인그레스 제어**

난수 발급 엔드포인트에 대해 클라이언트 IP별 요청 빈도를 추적하는 인메모리 슬라이딩 윈도우 알고리즘을 적용한다. 단일 IP 기준 초당 10회를 초과하는 요청에 $429$ 상태 코드와 재시도 응답 헤더를 반환하여 과도한 난수 생성과 인메모리 상태 누적을 억제한다. 락 내부 연산은 `collections.deque`의 양방향 큐를 기반으로 구성하며, 만료된 항목은 요청 처리 시점에 지연 제거(Lazy Expiration)되어 별도의 순회 연산 없이 $O(1)$ 시간 복잡도를 달성한다.

2. **저장소 수명주기 및 가비지 컬렉션**

발급된 모든 난수에 60초의 유효기간(TTL)을 부여한다. 멀티스레드 환경의 원자적 접근 제어를 위해 상호 배제 락을 적용하며, 특히 난수의 조회와 삭제를 단일 임계 구역 내에서 원자적으로 처리하여 동시 요청에 의한 이중 소비(TOCTOU, Time-of-Check Time-of-Use) 경합 조건을 원천적으로 차단한다. 또한 만료된 항목을 주기적으로 삭제하는 백그라운드 태스크를 구동하여 미소비 난수의 장기 누적에 의한 메모리 증가를 방지한다.
...
제안한 PoP 역방향 프록시 게이트웨이의 성능 및 보안 유효성을 실증하기 위해 Python 3.13과 FastAPI 프레임워크 기반으로 프로토타입 시스템을 구축하였다. 게이트웨이는 비동기 I/O 이벤트 루프 상에서 구동하고, 백엔드 마이크로서비스 환경을 모사한 업스트림 서버와 루프백 인터페이스로 연결하였다.

클라이언트 측 하드웨어 서명 환경은 W3C WebAuthn L3 규격에 부합하는[4] NIST P-256 비대칭 키 쌍을 기준으로[6] 소프트웨어 시뮬레이션 클라이언트로 구현하였다. 실험은 Intel Core i7 프로세서 및 16 GB 메모리 환경에서 수행하였다. 단일 프로세스 기준 100회의 독립적인 연속 HTTP 트랜잭션을 실행하여 평균값을 도출하였다[11].
...
실험 결과, 게이트웨이는 JWT만을 탈취한 세션 하이재킹 공격을 1단계 서명 헤더 검증에서 차단하였다. 스니핑을 이용한 재전송 공격 역시 3단계 난수 캐시 소모 메커니즘을 통해 암호 연산 이전에 배제하였다. 또한 HTTP 메서드나 본문 데이터가 변조된 패킷은 정규화 다이제스트 불일치로 판단하여 서명 검증 단계에서 $403$ 상태 코드를 반환하였다. 이를 통해 Fast-Fail 파이프라인이 무효한 패킷을 고비용 암호 연산 이전에 차단함을 확인하였다.
...
|---|---|---|---|---|
| **Mode A (표준 JWT)** | 4.36 ms | 8.11 ms | 0 Bytes | 기준 (Baseline) |
| **Mode B (RSA-2048)** | 4.87 ms | 7.61 ms | 499 Bytes | +11.85 % |
| **Mode C (ECDSA P-256)** | 7.27 ms | 15.02 ms | 243 Bytes | +66.96 % |

모드 C는 모드 A 대비 평균 2.91 ms(상대 오버헤드 66.96%)의 지연시간 부담을 나타냈다. 절대 지연시간 기준으로는 7.27 ms로서 실시간 웹 트래픽 환경에서 허용 가능한 수준으로 판단된다. 95백분위 지연시간에서는 모드 C가 15.02 ms로 측정되어 기준 모드인 A(8.11 ms) 및 B(7.61 ms) 대비 소폭 상승하였으나, 20 ms 미만의 범위를 유지하여 안정적인 응답 분포를 나타냈다.

페이로드 측면에서는 모드 B가 256 Bytes의 고정 서명 크기와 Base64 인코딩 팽창으로 인해 499 Bytes의 헤더 오버헤드를 나타낸 반면, 모드 C는 64 Bytes의 서명 좌표 구조를 통해 오버헤드가 243 Bytes로 낮게 나타났다. 본 측정은 서명 관련 헤더(`X-FIDO2-Credential-ID`, `X-FIDO2-Signature`, `X-FIDO2-Nonce`, `X-FIDO2-Timestamp`)를 대상으로 하며, 양 알고리즘에 공통으로 추가되는 `X-Body-Digest` 헤더는 비율 비교에서 제외하였다. 이는 RSA 대비 약 51.3%의 서명 헤더 페이로드 절감 효과에 해당한다.
...
기존 게이트웨이 구조는 CPU 집약적인 타원곡선 서명 연산 시 이벤트 루프가 차단되는 병목을 지닌다. 이를 `anyio.to_thread.run_sync()` API를 통해 비동기 스레드 풀로 위임함으로써 이벤트 루프를 차단하지 않고 CPU 집약적 연산을 수행할 수 있으며, `cryptography` 라이브러리가 ECDSA 연산 중 전역 인터프리터 락(GIL)을 해제하는 특성과 결합되어 단일 프로세스에서도 멀티 코어 병렬 처리를 실현하였다. 또한 인그레스 제어기의 락 내부 연산을 $O(1)$로 재설계하여 임계 영역 점유 시간을 단축하였다. 결과적으로 최적화 아키텍처는 기존 대비 초당 처리량(RPS)을 437 % 향상시켰으며 95백분위 지연시간(P95)을 94 % 감소시켰다. 아울러 임계치 초과 시에도 응답 지연 없이 $429$ 상태 코드를 즉각 반환하는 것으로 나타나, 대규모 트래픽 방어에 적합한 것으로 판단된다.
...
본 연구의 결과는 크게 세 가지 측면에서 의미를 가진다.

첫째, 하드웨어 격리 기반의 PoP 게이트웨이 아키텍처는 개인키를 호스트 운영체제로부터 물리적으로 분리함으로써, 기존 소프트웨어 기반 DPoP 방식이 지닌 키 유출 위험을 아키텍처 수준에서 해소한다(III, IV장). 둘째, Fast-Fail 파이프라인과 $O(1)$ 복잡도의 인그레스 제어를 통해 무효 요청과 대량 트래픽을 비대칭 암호 연산 이전 단계에서 차단함으로써, 자원 고갈 공격에 대한 방어와 시스템 자원 효율성을 함께 확보하였다(IV, V-3장). 셋째, 성능 평가 결과(V-4, V-5장)는 제안 아키텍처가 보안성 강화에 따른 성능 저하를 실용적 수준으로 제한하면서도, 동시성 최적화를 통해 고부하 환경에서의 처리 성능을 확보할 수 있음을 뒷받침한다.

종합하면, 본 연구는 하드웨어 기반 신원 검증과 경량 암호 알고리즘 선택을 통해 보안성과 성능 오버헤드 간의 균형을 확보할 수 있음을 시사한다.
...
2. **단일 노드 인메모리 아키텍처의 확장성 제약**: 제안 시스템의 난수 캐시와 슬라이딩 윈도우 인그레스 제어기는 단일 프로세스의 스레드 안전성을 기준으로 설계하였다. 따라서 로드 밸런서 기반의 다중 게이트웨이 인스턴스로 수평 확장할 경우 인스턴스 간 상태 불일치 문제가 발생할 수 있다.

3. **인증기 증명(Attestation) 및 구조적 검증의 간소화**: 본 프로토타입은 등록 단계의 복잡도를 줄이기 위해 하드웨어 진위를 판별하는 Attestation 구조 및 `signCount` 기반의 인증기 복제 탐지를 간소화하여 에뮬레이션하였다. 

4. **신뢰 경계 및 운영 정책의 부재**: 게이트웨이와 업스트림 간의 평문 통신 보호(mTLS 등), HTTP Method Override와 같은 헤더 변조 방어, 레거시 환경을 고려한 다운그레이드(Fallback) 정책 설계가 반영되지 않았다. 

5. **평가 방법론의 변수 통제 한계**: V-5절의 성능 향상은 비동기 I/O 위임과 락 재설계가 복합적으로 작용한 결과이므로, 개별 요인의 기여도를 분리하는 절제(Ablation) 실험이 추가로 필요하다. 또한 난수 발급의 RTT 지연이 측정에서 제외되었으므로, 실제 지연시간 오버헤드는 본 측정치보다 클 수 있다.
