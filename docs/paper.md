...
3. **JWT 바인딩 검증**: 토큰 내의 `cnf` (Confirmation) 클레임을 확인하여 현재 제시된 공개키와 JWT가 올바르게 바인딩되어 있는지 확인한다.

4. **비대칭 암호 서명 검증**: 클라이언트가 헤더에 명시한 Claimed Digest를 포함하여 ECDSA P-256 서명의 수학적 유효성을 먼저 검증한다.

5. **본문 다이제스트 스트리밍 대조**: 이전 단계를 통과한 유효한 서명의 요청에 한하여, 본문을 스트리밍하며 실제 해시값과 Claimed Digest의 일치 여부를 대조한다. 이를 통해 무효한 서명으로 대용량 데이터를 전송하는 자원 고갈 공격을 차단한다.
...
모드 C는 모드 A 대비 평균 1.10 ms(25.65%)의 지연시간 오버헤드를 나타냈다. 이는 실시간 웹 트래픽 환경에 적용 가능한 수준의 성능으로 판단된다. 상위 5 % 요청의 최대 지연시간인 95백분위 지연시간에서는 모드 C가 6.54 ms로 모드 A와 동일한 안정적인 수치를 기록하였다. 반면 모드 B의 95백분위 지연시간은 9.19 ms로 가장 높게 측정되었다.

페이로드 측면에서는 모드 B가 256 Bytes의 고정 서명 크기와 Base64 인코딩 팽창으로 인해 499 Bytes의 헤더 오버헤드를 나타낸 반면, 모드 C는 64 Bytes의 서명 좌표 구조를 통해 오버헤드가 252 Bytes로 낮게 나타났다. 이는 RSA 대비 50%의 페이로드 절감 효과에 해당한다.
